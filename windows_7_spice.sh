...
SOCK="$HOME"/.windows_7_spice.sock  # spice socket file
...
tools="qemu-system-x86_64 remote-viewer smbcontrol"
for i in $tools; do
  if ! which -s "$i"; then
...
eval qemu-system-x86_64 -k ru \
  ...
  -cpu host,hv_relaxed,hv_spinlocks=0x1fff,hv_vapic,hv_vpindex,hv_synic,hv_time,hv_stimer,hv_ipi \
  -smp cpus=$CPUS -m size=$RAM \
  -vga $VGA \
  -nographic -rtc base=localtime,clock=host \
  -spice unix=on,addr=\"$SOCK\",disable-ticketing=on,gl=on \
  ...
  -audiodev pa,driver=pa,id=pa0 -device AC97,audiodev=pa0 \
  -name \"$NAME\" \
...
# Wait until QEMU creates the spice socket
echo "Wait for the QEMU to create the spice socket..."
while :
do
  if [ -e "$SOCK" ]; then
    break
...
remote-viewer --auto-resize=always -z $ZOOM "spice+unix://$SOCK" & VIEWER_PID=$!
...
# Wait until the QEMU and the Spice viewer are done
echo "Waiting for QEMU and remote-viewer to exit..."
while :
do
  if ! ps -p $QEMU_PID > /dev/null & \
    ! ps -p $VIEWER_PID > /dev/null
  then
    break
  fi
  sleep .25
