...
NAME="Windows $WIN_VERSION"  # VM name
...
SOCK="$HOME"/.windows_"$WIN_VERSION"_spice.sock  # spice socket file
...
if [ $SPICE -eq 1 ]; then
  # Wait until the Spice viewer is done
  echo "Waiting for remote-viewer to exit..."
  while :
  do
